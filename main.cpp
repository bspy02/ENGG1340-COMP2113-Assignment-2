...
//        false otherwise
bool isinside(AABB box, int x, int y) 
{ 
    return (x > box.min[0] and x < box.max[0] and y > box.min[1] and y < box.max[1]);
} 
bool IsOverlap(AABB p, AABB q) {
  // --- YOUR CODE HERE (Task 3) ---
    return isinside(p,q.min[0],q.min[1]) or isinside(p,q.min[0],q.max[1]) or isinside(p,q.max[0],q.min[1])or isinside(p,q.max[0],q.max[1]);
}
